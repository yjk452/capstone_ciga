...
def build_gate_input(
    d: torch.Tensor,
    *,
    cam_center: torch.Tensor,      # [3]
    cam_forward: torch.Tensor,     # [3] (unit vector 권장)
    d_low: float = 0.2,
    d_high: float = 30.0,
    d0: float = 5.0,
    dw: float = 0.5,
    cam_center_scale: float = 50.0,
) -> torch.Tensor:
    """
    in_dim=7 input:
      x = [ u(d), cam_center(3)/scale, cam_forward(3) ]
    """
    u = d_to_u(d, d_low=d_low, d_high=d_high, d0=d0, w=dw)  # [N]

    cam_center = cam_center.view(-1)[:3]
    cam_forward = cam_forward.view(-1)[:3]

    N = d.shape[0]
    c = (cam_center.unsqueeze(0).expand(N, 3)) / float(cam_center_scale)  # [N,3]
    f = cam_forward.unsqueeze(0).expand(N, 3)                              # [N,3]

    return torch.cat([u.unsqueeze(-1), c, f], dim=-1)  # [N,7]


class GateMLP(nn.Module):
    def __init__(self, in_dim: int = 7, L_max: int = 2, hidden: int = 64):
        super().__init__()
        self.L_max = L_max
...
class GateLUT(nn.Module):
    """
    선택사항: MLP를 LUT로 distill해서 빠르게 쓰고 싶을 때.
    LUT 입력은 (u, ..)를 직접 만들고 lookup해야 하므로 여기서는 유지하되,
    in_dim=7과 직접 1:1 연결되는 구조는 아님.
    """
    ...
    def update_from_mlp(self, mlp: GateMLP, grid_u: torch.Tensor, grid_v: torch.Tensor):
        ...
        g = mlp(inp)  # 이 부분은 in_dim=2일 때만 의미 있음
...
class AdaptiveSHLoss(nn.Module):
    """
    (옵션) 게이트 weights 규제용 loss. 입력은 sh_weights + distances 위주라 그대로 사용 가능.
    """
