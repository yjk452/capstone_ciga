...
class SepDepthTrim2DGSRenderer(Renderer):
    ...
    - Keeps trimming hooks / allmap outputs.
    - Adds gate MLP outputs: shs_raw, shs_gated, sh_weights, adaptive_sh_info.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        in_dim = int(self.mlp_cfg.get("in_dim", 2))
    ...
    def forward(
        ...
        **kwargs,
        ...
        # SH features (v2 uses SH directly)
        shs = pc.get_features
        ...
        distances = torch.linalg.norm(means3D - cam, dim=1)
        ...
        rets.update(
            {
                ...
                "adaptive_sh_info": {"distances": distances},
            }
        )
    ...
    def shs_weight_MLP(self, rasterizer, shs, VC, means3D, L=None):
        ...
        Uses GateMLP from sh_core (3rd file).
        """
        if shs is None:
            self._last_band_w = None
            return shs
        ...
        # build gate input: [u(d), v(cos(nadir))]
        ...
        x = build_gate_input(d, cos_angle, d_low=d_low, d_high=d_high, d0=d0, dw=dw)  # [M,2]
