...
def get_free_memory_mib_list():
    ...
    for i in range(n):
        ...
        free_list.append(mem.free // (1024**2))  # MiB
    nvml.nvmlShutdown()
    return free_list
...
def train_a_partition(
    ...
    run_func = functools.partial(subprocess.run, env=os.environ.copy())
...
if __name__ == "__main__":
    ...
    if len(srun_args) == 0:
        num_gpus = len(get_free_memory_mib_list())
        max_workers = min(num_blocks, max(1, num_gpus))
        with ProcessPoolExecutor(max_workers=num_blocks) as executor:
            for block_id in range(num_blocks):
                gpu_available = False
                fail_cnt = 0
                while not gpu_available:
                    free_mib = get_free_memory_mib_list()

                    print(f"block ({block_id%config.data.parser.init_args.block_dim[0]}, {block_id//config.data.parser.init_args.block_dim[1]})")
                    print(f"[GPUCHK free_mib] : {free_mib}")

                    if len(free_mib) > 0 and max(free_mib)>= MIN_FREE_MIB:
                        selected_gpu = int(np.argmax(free_mib))
                        gpu_available = True
                    elif fail_cnt > 90:
                ...
                executor.submit(train_a_partition, args, training_args, srun_args, block_id, selected_gpu)
